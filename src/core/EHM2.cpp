...
namespace ehm
{
    namespace core
    {
        ...
        std::tuple<Eigen::MatrixXd, double> EHM2::computeAssociationMatrixAndLikelihood(const EHM2NetPtr net, const Eigen::MatrixXd &likelihood_matrix)
        {
            int num_tracks = likelihood_matrix.rows();
            int num_detections = likelihood_matrix.cols();
            int num_nodes = net->getNumNodes();
            ...
            for (int track = 0; track < num_tracks; track++)
            {
                ...
                // Normalise
                likelihood = a_matrix(track, Eigen::all).sum();
                a_matrix(track, Eigen::all) /= a_matrix(track, Eigen::all).sum();
            }
